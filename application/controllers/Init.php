...
class Init extends CI_Controller
{
     ...
     * if he is not redirect him to the signin class
     * Init constructor.
     */
    ...
    public function only_admin() {
        if(!is_admin(get_property($this->logged_member, "id"))) {
            redirect("forbidden");
        }
    }

    /**
     * if the logged in user not member go to forbidden
     */
    public function only_member() {
        if(!logged_is_member()) {
            redirect("forbidden");
        }
    }
    ...
    public function only_owner($target_id) {
        if(!is_owner(get_property($this->logged_member, "id"), $target_id)) {
            redirect("forbidden");
        }
    }
    ...
    public function only_logged_have_permission($permission = "", $owner_can_edit = false) {
        if(!logged_have_permission($permission, $owner_can_edit, $this->logged_member)) {
            redirect("forbidden");
        }
    }
    ...
    public function only_logged_have_permissions($permissions = array(), $owner_can_edit = false) {
        if(!logged_have_permissions($permissions, $owner_can_edit, $this->logged_member)) {
            redirect("forbidden");
        }
    }
     ...
     * @param bool $target
     */
    public function only_logged_can_edit_admin($permissions = array(), $target = false) {
        $have_permission = logged_have_permissions($permissions, $target, $this->logged_member);
        if (!((is_admin(get_property($this->logged_member, "id")) || !is_admin($target)) && $have_permission)) {
            redirect("forbidden");
        }
