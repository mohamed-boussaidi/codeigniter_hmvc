<?php echo load_js(array("assets/js/cropbox/cropbox-min.js")) ?>
<div style="z-index: 999999 !important;" class="modal fade" id="cropModal" tabindex="-1" role="dialog" aria-labelledby="cropModal">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-body">
                <div class="crop-box">
                    <div class="thumb-box"></div>
                    <div class="spinner" style="display: none"><?php echo plang("loading") ?></div>
                </div>
            </div>
            <div class="modal-footer clearfix">
                <button id="image-zoomout-button" type="button" class="btn btn-default pull-left mr10">
                    <i class="fa fa-minus"></i>
                </button>
                <button id="image-zoomin-button" type="button" class="btn btn-default pull-left">
                    <i class="fa fa-plus"></i>
                </button>
                <button type="button" class="btn btn-default" data-dismiss="modal">
                    <i class="fa fa-times"></i> <?php echo plang("close") ?>
                </button>
                <button id="image-crop-button" type="button" class="btn btn-primary" data-dismiss="modal">
                    <i class="fa fa-check-circle"></i> <?php echo plang("crop") ?>
                </button>
            </div>
        </div>
    </div>
    ...
    function showCropBox(element) {
        ...
        if (file) {
            ...
            if (fileTypes.indexOf(file.type) === -1) {
                appAlert.error('<?php echo plang("invalid_file_type") ?>');
                appLoader.hide();
                return false;
            } else if (file.size / 1024 > AppHelper.settings.profileImageSize) {
                appAlert.error('<?php echo plang('max_file_size_message', array(get_setting("profile_image_max_size"))) ?>');
                appLoader.hide();
                return false;
            }
