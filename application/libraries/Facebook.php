...
Class Facebook
{
    ...
    public function __construct(){
        ...
        $this->configs['app_id'] = $this->config->item('facebook_app_id');
        $this->configs['app_secret'] = $this->config->item('facebook_app_secret');
        $this->configs['default_graph_version'] = $this->config->item('default_graph_version');
        $this->configs['auth_on_load'] = $this->config->item('facebook_auth_on_load');
        $this->configs['login_redirect_url'] = $this->config->item('facebook_login_redirect_url');
        $this->configs['logout_redirect_url'] = $this->config->item('facebook_logout_redirect_url');
        $this->configs['permissions'] = $this->config->item('facebook_permissions');
        $this->configs['login_type'] = 'web';
        if (function_exists('get_facebook_api_settings')) {
            $configs = get_facebook_api_settings();
            $this->configs['permissions'] = separated_to_array($configs['facebook_permissions']);
            $this->configs['app_id'] = $configs['facebook_app_id'];
            $this->configs['app_secret'] = $configs['facebook_app_secret'];
            $this->configs['login_redirect_url'] = $configs['facebook_login_redirect_url'];
            $this->configs['logout_redirect_url'] = $configs['facebook_logout_redirect_url'];
        }
        if (!isset($this->fb)){
            $this->fb = new FB([
        ...
        switch ($this->configs['login_type']){
            case 'js':
        ...
        if ($this->configs['auth_on_load'] === TRUE){
    ...
    public function login_url(){
        ...
        if($this->configs['login_type'] != 'web'){
            return '';
        }
        // Get login url
        return $this->helper->getLoginUrl(
            base_url() . $this->configs['login_redirect_url'],
            $this->configs['permissions']
        );
    }
    ...
    public function logout_url(){
        ...
        return $this->helper->getLogoutUrl(
            ...
            base_url() . $this->configs['logout_redirect_url']
        );
    }
    ...
    private function authenticate(){
        ...
        if(!$access_token){
            try{
                $access_token = $this->helper->getAccessToken(base_url() . $this->configs['login_redirect_url']);
            }catch (FacebookSDKException $e){
