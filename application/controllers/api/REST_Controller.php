...
abstract class REST_Controller extends CI_Controller
{
     ...
     * 4 : file unzip error
     * 5 : tags format not supported only serialized php arrays accepted
     * 6 : empty tags, tags array ex : array(array('name' => 'tag_name', 'value' => 'tag_value'))
     * 7 : config file not found
     * 8 : error while reading the file
     * 9 : error while writing the tmp config file
     * 10 : error while deleting the file
     * 11 : error while renaming the tmp file
     * 12 : sql file not found
     * 13 : error while executing sql file
     * 14 : creating new demo error
     * 15 : user have an active demo
     * default : unrecognized error
     * output api error code msg as json
     * @param int $error_code
     * @param string $info
     * @param array $more
     */
    public function error($error_code = 0, $info = "", $more = array()) {
        $error = ["success" => false, "error_code" => $error_code, "info" => $info, "more" => $more];
        switch ($error_code) {
            case 1 : $error["message"] = "args_error"; break;
            default : $error["message"] = "error_occurred"; break;
        }
        $this->response($error,200);
    }

	/**
	 * intercept php errors and parse them
	 * you should use it between try catch
	 * @param string $e
	 */
    public function phpError($e = "") {
        $error = ["success" => false, "error_code" => 0];
        $error["message"] = "Line : ".$e->getLine()." "."File : ".$e->getFile()." ".$e->getMessage();
        $this->response($error,200);
    }
     ...
     * @param string $url
     */
    public function success($data = array(), $msg = "", $url = "") {
    ...
    protected function _detect_api_key()
    {
        ...
        {
            $this->rest->db->where(config_item('rest_key_column'), $key);
            if ( ! ($row = $this->rest->db->get($this->db->dbprefix(config_item('rest_keys_table')))->row()))
            {
                return FALSE;
            }
